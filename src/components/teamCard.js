...
const TeamCard = ({ imageSrc, fullName, position, description, skills, isLastCard, isExpanded, onExpand, expandedIndex, onMouseEnter, onMouseLeave }) => {
  ...
  return (
    <div
      ...
      className={`
        ...
        ${isLastCard ? '' : 'border-b lg:border-b-0 lg:border-r border-suloke-white/20 lg:border-suloke-black/20'}
        ${isExpanded
          ? 'h-[500px] lg:w-[50%]'
          : expandedIndex === null
            ? 'h-[244px]'
      ...
      <div className="relative flex flex-col py-4 px-2 z-10 max-w-[55%] h-full lg:max-w-full lg:justify-between">
        <div className="flex flex-col">
          <div className="flex flex-row items-center gap-2 pl-3">
            ...
            <h2 className={`text-3xl transition-opacity duration-300 ${isExpanded ? 'opacity-100' : 'opacity-0'}`}>{lastName}</h2>
          </div>
          <h3 className={`text-xl text-suloke-white/50 pl-5 transition-all duration-300
        ${isExpanded ? 'opacity-100' : 'lg:opacity-0'}
        `}>
            {position}
          </h3>
        </div>
        <div className={`transition-opacity rounded-lg duration-300 overflow-hidden p-5 mt-4 lg:bg-suloke-black/70
              ${isExpanded ? 'opacity-100 lg:delay-200' : 'opacity-10 lg:opacity-0 lg:delay-0 lg:duration-0'}`}>
          <p className="text-suloke-white/80 mb-4">
            {description}
          </p>
          <div className={`flex flex-wrap gap-2 transition-all duration-300 ${isExpanded ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
